...
-- ================================
local function M.file_diff_input()
...
local function run_folder_diff(first, second, callback)
  local exclude = {
    ...
    "--exclude=vendor", "--exclude=__pycache__", "--exclude=*.egg-info",
    "--exclude=env", "--exclude=venv", "--exclude=Library"
  }
  local args = { "-qr" }
  ...
  handle = vim.loop.spawn("diff", {
    ...
    vim.schedule(function() callback(diffs, items) end)
  end)
...
local function select_pair(diffs, items, callback)
  ...
  if ok then
    ...
    vim.ui.select(items, { prompt = "Select file pair to diff:" }, function(choice, idx)
      if choice and idx then callback(diffs[idx], idx) end
    end)
  end
end
...
function M.folder_diff()
  ...
  local fd_cmd = [[fd --type d --follow --exclude .git --exclude '.*' --exclude Library . ]] ..
    vim.fn.expand("~") .. [[ | awk -F/ '{print NF,$0}' | sort -n | cut -d' ' -f2-]]
    ...
    fzf.fzf_exec(fd_cmd, {
      ...
      actions = {
        ["default"] = function(selected)
          ...
          if not sel:match("^/") then sel = vim.fn.expand("~") .. "/" .. sel end
